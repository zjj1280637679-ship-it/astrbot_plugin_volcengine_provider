...
- 当前回执若明确给出某字段（包括 `False`），只在**本次 Source 的 Dashboard 响应**里覆盖同名旧展示信息；不会写回 AstrBot 全局 `LLM_METADATAS`，不会成为路由事实。
...
如果请求已经到达火山并由上游返回错误，则继续走 AstrBot/OpenAI SDK 原生错误与 fallback 机制。插件只区分信息来源，不替用户把某次上游反馈写成永恒模型能力，也不复制 AstrBot 的路由器。
