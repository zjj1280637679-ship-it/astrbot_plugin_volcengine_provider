...
- **视频勾选回到模型卡，但只属于正确对象。** 火山方舟普通 API 与 Agent Plan 的模型编辑弹窗会在 AstrBot 原有“模型能力”复选框中增加“视频”；OpenAI、Google 等其他供应商的弹窗保持原样，不出现火山字段。
- 这次隔离不再修改共享 schema 实例。插件只在 Dashboard 已经复制出当前弹窗 schema、且同时掌握所选 Provider Source 类型后，改写这一份私有副本；因此“共享结构”得到复用，“共享实例”不再被污染。
- 勾选“视频”会保存到当前模型卡的标准 `modalities` 列表，并在火山模型卡保存边界同步到兼容运行字段 `volcengine_video_input_enabled`。它表达的是你的请求传输选择，不是插件推断所有火山模型都支持视频。
- 0.1.18 的 Source 页面视频开关/集中选择器正式退役；0.1.19 的三态开关继续作为已经打开的旧页面兼容输入。视频质量仍可单独选择“压缩 / 原画”，关闭视频不会丢失上次质量偏好。
- Dashboard 目前没有官方 Provider 专属模型 schema 扩展钩子。插件因此以可撤销包装读取唯一匹配的 Provider 弹窗脚本，生成仅存于临时目录的变换副本并按需返回；不会修改 AstrBot 磁盘文件。若未来 Dashboard 结构无法唯一识别，插件保留原页面、跳过这项可选 UI，两个 Provider 仍照常注册，不以版本或结构未知为由禁止加载。
- 已在批准的 Windows `codex测试实例` 上观察到：普通 Ark 显示并持久化视频勾选，外国 Provider 不显示视频，AstrBot 原始静态资源未被改写。正式最小运行包、双版本原生安装门禁和商场更新/日志仍以本次发布流程的实时结果为准。
