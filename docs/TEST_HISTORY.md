...
| 0.1.20 release preparation (2026-08-13) | Source-scoped model-dialog video option | Local contracts found exactly one provider-dialog clone boundary in AstrBot 4.27.2, added video only for Ark/Agent Plan, preserved OpenAI/Google options, restored the wrapped resolver on release, and removed the plugin-owned temporary copy. A real approved Windows AstrBot instance showed Ark Video, persisted it after save/reopen, showed no Video on a foreign provider, and served one transformed asset marker while the original disk asset retained zero markers | The private dialog clone plus selected Source type is a sufficient isolation boundary for the observed host. This proves presentation/config isolation, not that every Volcengine model supports video. Marketplace-installed runtime and Agent Plan visual observation remain separate frontier items until directly observed |
