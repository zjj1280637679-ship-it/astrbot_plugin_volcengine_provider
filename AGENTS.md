...
**0.1.19 is the active development release candidate, not a published runtime.** Draft PR #7 keeps the 0.1.18 Provider Source UI frozen and adds bilingual per-model horizontal request settings only to saved Ark / Agent Plan model edit dialogs. The candidate branch metadata is 0.1.19 so the exact allow-list package can be validated before merge; `main`, stable `runtime`, and the external AstrBot Store remain on the published 0.1.18 state until the PR is explicitly approved, merged, and the post-merge publisher completes.

The 0.1.19 candidate adds Video Input Mode (Off / Compressed / Original), Thinking Mode, Reasoning Effort, Temperature, Top P, Max Output Tokens, Stop Sequences, Frequency Penalty, and Presence Penalty. Explicit horizontal values override the same `custom_extra_body` request keys; empty rows inject nothing. Foreign model cards receive no Volcengine fields. The first unsaved add-model dialog remains host-native because AstrBot constructs it client-side; the new rows appear after the model is saved and reopened. No generic Thinking Budget row is invented without a verified Ark mapping.

The candidate has already passed real AstrBot 4.27.2 Dashboard observation for Ark / Agent Plan / foreign isolation, and the dual 4.26.1 / 4.27.2 runtime contracts include a real ffmpeg compression round trip. These are pre-release validation facts only. The final 0.1.19 publication proof still requires merge to `main`, a successful 0.1.19 Runtime Distribution Gate on that exact main SHA, the publisher's pre-promotion native-install matrix, leased `runtime` promotion, and the blocking post-promotion matrix.

**0.1.18 remains the currently published repository/runtime release.** PR #4 was squash-merged to `main` at `22444f47154f4f88ff3157d6e6ffcce9ad2689f0`; main gate run `31589741300` passed. Publisher run `31589815606` passed prepare, all four pre-promotion native-installer cells, promotion, and all four blocking post-promotion cells. The stable `runtime` branch is `4586aa2eb573eb97a72baaaa152c727e3b35530e`, contains 21 runtime blobs byte-for-byte identical to the gated artifact, and reports metadata version 0.1.18.
