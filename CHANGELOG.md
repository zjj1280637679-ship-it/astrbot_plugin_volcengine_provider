...
- **默认分支恢复完整运行闭包**：把已知正常的 0.1.32 运行树中 `_conf_schema.json`、可配置音视频限制、超限图片压缩与缓存命中观测逐文件对账回 `main`。此前 0.1.33 只完成了发布地址/文档归位，默认分支缺少这些已声明功能，不能作为完整安装包。
- **修复 AstrBot 4.27.4 模型卡 Video 消失**：Dashboard 资产桥兼容模型构建器同时存在和不存在 `,reasoning:<fn>` 片段的两种宿主产物；三结构边界仍要求各精确命中一次，部分命中继续 fail closed，foreign Provider 不会获得插件字段。
- **补齐图片压缩配置契约**：超限图片在首次 JPEG 编码前按 `image_compress_max_size` 等比缩小，不放大小图；回归同时验证字节上限、最长边、未触发图片不改写与 `detail` 字段保留。
- **收窄缓存观测职责**：移除按模型名前缀猜测上下文容量和并非请求耗时的 `ms` 指标；上下文长度错误只记录诊断，重试与历史裁剪继续完全委托 AstrBot。
- **补验 AstrBot 4.27.4 音频解析变更**：使用宿主真实编码器生成 AMR 与 Tencent Silk，把内容故意伪装为 `.wav` 后走实际 `MediaResolver` 与插件 Ark 归一化，均得到 RIFF/WAVE、16 kHz、单声道、PCM16；不调用付费 API。
- **保持运行语义**：继续使用已验证的 0.1.32 Provider、音频、视频、图片、缓存实现；不改变普通 Ark / Agent Plan 固定端点，不增加跨通道回退，也不把传输反馈升级成模型能力真值。
- **单一发布源**：`main` 根目录成为唯一 AstrBot 安装与版本真值；旧 `runtime` 只保留历史恢复证据。移除已退役的双分支生成/对账工具，新增默认分支完整性、版本账本、配置 schema、语法、大小与高置信密钥模式检查。
- **兼容范围**：`astrbot_version` 保持为 `>=4.26.1`，不新增未来版本硬拒载。模型卡 Video 与生命周期 CI 覆盖 AstrBot 4.27.3 / 4.27.4；测试范围不等于对尚未验证版本的行为保证。

## 0.1.33（发布身份已失效）

- 将 `metadata.yaml.repo` 和 README 改回标准仓库根地址，并退役自定义 `runtime` 发布工作流。
- 该版本没有把 0.1.28—0.1.30 的完整运行文件同步回默认分支，导致 `main` 的版本声明、README 承诺与实际安装内容不一致；由 0.1.34 以更高版本修复，不复用或改写 0.1.33。

## 0.1.32

- 对 0.1.31 紧急恢复后的 `main` / `runtime` / README / HOT 状态做版本事实对账，不改变当时已知正常的运行 payload。
- 该版本的历史 `runtime@89c5512` 是 0.1.34 恢复缺失文件时使用的逐路径参考，但不再是当前安装或发布入口。

## 0.1.31

- 在一次内部测试通过但现场破坏 Video/模型卡 UI 的实验后，紧急恢复到 0.1.30 已知正常功能树并提升版本身份。
- 废弃实验未合入当前 0.1.34；恢复只采用后来确认正常的 0.1.32 运行 payload。
