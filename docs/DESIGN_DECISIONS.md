...
Because selector translation updates per-card in-memory truth before the host Source upsert completes, the bridge snapshots the complete model-card list first. If the host save rejects or raises, the list is restored and the original error continues outward; a failed Source edit cannot leave a half-applied plugin state.
