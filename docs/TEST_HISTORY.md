...
| Unreleased (2026-08-12) | Source video UI isolation | The Source save contract passed the real AstrBot 4.26.1 and 4.27.2 service matrix (L3). A separate real 4.27.2 Dashboard DOM run passed L4: Ark/Plan masters were 1/1; their opened selectors were 1/1 and contained only their own 2/1 cards; close hid the selector, reopen preserved selection with 0 API requests; foreign was 0/0; Ark/Plan/foreign generic model dialogs contained 0 canonical, retired temporary, or new temporary video fields; `pageErrors=[]` | The owned-Source configuration boundary, conditional selector presentation, client-side hide/reopen preservation, and generic-card/foreign isolation worked in the observed host/UI conditions. This does not prove model video capability, Ark runtime acceptance, or the QQ product path |
| Unreleased (2026-08-12) | 0.1.17 upgrade residue and Source-save rollback | Contract regressions reproduced AstrBot 4.26.1 live-schema residue and proved precedence `canonical > exact-Source retired 0.1.17 bool > older per-card > legacy Source bool including false > modalities`; wrong-Source and foreign fields were not promoted, all retired/temporary/wrong-layer fields were removed, and `modalities` stayed unchanged. A service regression forced host `upsert_provider_source` failure after selector translation and observed restoration of the complete pre-call model-card list with the original error re-raised | The known 4.26.1/0.1.17 upgrade artifact preserves only identity-matched user intent, and a failed Source save cannot leave plugin-created half-applied in-memory card state. These are migration/transaction-boundary results, not model capability or media-path evidence |
