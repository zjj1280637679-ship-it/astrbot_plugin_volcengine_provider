...
Interpretation:
- PASS => model-ID -> AstrBot provider card -> persisted config -> real DeepSeek
  request is causally reachable; any separate live-model-list failure belongs to
  discovery/rendering, not to the model ID being unusable.
...
def persisted_manual_evidence(source_id: str) -> dict[str, Any]:
    ...
    if source.get("key") != ENV_REFERENCE:
        raise AssertionError("DeepSeek Source did not persist the environment key reference")
